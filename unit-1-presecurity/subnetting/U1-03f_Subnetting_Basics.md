...
A /26 provides 62 usable hosts, so it can fit every department. However, it would waste a lot of address space for the smaller departments.

The smallest suitable subnet for each department is:
...
Remove leading zeroes and compress the consecutive zero groups:
...
The :: notation can replace one consecutive run of zero groups in an IPv6 address. It can only be used once in an address because otherwise the original number of groups would be ambiguous.
