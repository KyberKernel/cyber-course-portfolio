...
Class    Default CIDR    Possible Networks    Hosts per Network
-----    ------------    -----------------    -----------------
A        /8              128 nets              16,777,214
B        /16             16,384 nets           65,534
C        /24             2,097,152 nets        254
...
A /30 provides exactly two usable host addresses, which makes it useful for point-to-point links.
...
10.10.0.75 is between the network and broadcast addresses, so it is a usable host address.
...
Four equal /26 subnets can be created because:
...
A /26 provides 62 usable hosts, so it fits all four departments.

However, smaller departments can use smaller subnets to avoid wasting addresses.
...
Department B: /27 provides 30 usable hosts, which is enough for 25 hosts.

Department C: /28 provides 14 usable hosts, which is enough for 10 hosts.

Department D: /30 provides exactly 2 usable hosts, making it suitable for a point-to-point link.
...
The :: notation replaces one consecutive run of zero groups. It can only be used once in an IPv6 address because using it more than once would make the original address ambiguous.
